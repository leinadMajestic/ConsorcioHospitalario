...
class WtSlideshowClass extends ObjectModel
{
	...
	public function getCaptionsByIdSlider($id_slide)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT cs.*
			FROM '._DB_PREFIX_.'wtslideshow_caption cs
			LEFT JOIN `'._DB_PREFIX_.'wtslideshow_caption_shop` css ON (css.`id_wtslideshow` = cs.`id_wtslideshow`)
			WHERE css.`id_wtslideshow` = '.$id_slide.' AND css.`id_shop` = '.$id_shop);
	}
