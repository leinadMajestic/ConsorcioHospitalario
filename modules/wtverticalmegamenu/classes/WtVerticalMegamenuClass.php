...
class WtVerticalMegamenuClass extends ObjectModel
{
	...
	public function getMenus()
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE wt.active = 1 AND wl.id_shop = '.$id_shop.' AND wl.id_lang='.$id_lang.' ORDER BY wt.position ASC, wt.id_wtverticalmegamenu ASC');
	}
	...
	public function getPositionByMenu($id_menu)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			SELECT ms.`position`
			FROM '._DB_PREFIX_.'wtverticalmegamenu_shop ms
			WHERE ms.`id_wtverticalmegamenu` = '.$id_menu.' AND ms.`id_shop` = '.$id_shop);
	}
}