...
class WtCouponPop extends Module
{
	...
	{
		if (!parent::install() || !$this->registerHook('displayHeader') || !$this->registerHook('displayBannerSpecial')) return false;
	...
	public function hookdisplayTopFooter($params)
	{
		...
		$this->prepared = true;
		return $this->display(__FILE__, 'wtcouponpop_topfooter.tpl');
	}
	public function hookdisplayBannerSpecial()
	{
		...
		if ($notshow >= 0)
		{
			if (!$this->isCached('wtcouponpop.tpl'))
			{
