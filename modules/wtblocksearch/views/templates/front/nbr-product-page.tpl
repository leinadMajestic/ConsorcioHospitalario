...
{if isset($p) AND $p}
	...
	{if $nb_products > $nArray[0]}
		<form action="{$link->getModuleLink('wtblocksearch', 'catesearch', array(), true)|escape:'html':'UTF-8'}" method="get" class="nbrItemPage">  
			...
			<div class="clearfix selector1">
				...
				{/if}
                {*}
				<label for="nb_item{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}">
					{l s='Show' mod='wtblocksearch'}
				</label>
                {*}
				{if is_array($requestNb)}
					...
					{/foreach}
				{/if}
				<select name="n" id="nb_item{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="form-control">
					{assign var="lastnValue" value="0"}
					{foreach from=$nArray item=nValue}
						{if $lastnValue <= $nb_products}
							<option value="{$nValue|escape:'html':'UTF-8'}" {if $n == $nValue}selected="selected"{/if}>{l s='Show' mod='wtblocksearch'}&nbsp;&nbsp;{$nValue|escape:'html':'UTF-8'}</option>
						{/if}
						{assign var="lastnValue" value=$nValue}
					{/foreach}
				</select>
				{*}<span>{l s='per page' mod='wtblocksearch'}</span>{*}
			</div>
		</form>
	{/if}
	<!-- /nbr product/page -->
