...
class WtThemeConfigurator extends Module
{
	...
	public function __construct()
	{
		...
		parent::__construct();
		$this->displayName = $this->l('WT Theme Config');
		$this->description = $this->l('Config Color, Load Script Js, Css');
	...
	public function hookdisplayHeader()
	{
		...
		$this->context->controller->addCss($this->_path.'views/css/frontend/wtthemeconfigurator.css');
		...
		if (Configuration::get('PS_LOGO_MOBILE'))
			$logo_footer = $this->context->link->getMediaLink(_PS_IMG_.Configuration::get('PS_LOGO_MOBILE').'?'.Configuration::get('PS_IMG_UPDATE_TIME'));
		else
				...
				if (isset($item['frontend']) && $item['frontend'] && $style == 'color')
				{
					if ($key != 'body_color')
					{
						if (is_string($item['selector']))
							$color_bgs['color_'.$key.''] = array($item['text'], $note , $item['default_val'], 'selector' => array($style => $item['selector']));
						else
							$color_bgs['color_'.$key.''] = array($item['text'], $note,  $item['default_val'], 'selector' => $item['selector']);
					}
					if ($key == 'body_color')
					$body_col = array($item['text'], 'color_'.$key.'', $note, $item['default_val'], 'selector' => array($style => $item['selector']));
				}
