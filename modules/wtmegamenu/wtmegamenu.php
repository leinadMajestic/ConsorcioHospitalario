...
class Wtmegamenu extends Module
{
	...
	public function getMenuInfo($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE pl.id_shop = '.$id_shop.' AND pl.id_lang = '.$id_lang.($active ? ' AND pc.`active` = 1' : ' ').' ORDER BY pc.position ASC, pc.id_wtmegamenu ASC'
		);
	}
	...
	protected function _postProcess()
	{
		...
		if (Tools::isSubmit('submitMenuItem'))
		{
			...
			if (Tools::getValue('id_wtmegamenu'))
			{
				...
				$pos_arr = $menu_item->getPositionByMenu((int)Tools::getValue('id_wtmegamenu'));
		...
		elseif (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_wtmegamenu'))
		{
			...
			$pos_arr = $menu->getPositionByMenu((int)Tools::getValue('id_wtmegamenu'));
			...
			foreach ($menus as $menu)
			{
				...
				else
				{
					if ($menu['type_link'] == 0)
					{
						...
						$menu['title'] = $menu_info['title'];
						$menu['selected_item'] = $menu_info['selected_item'];
						$int_width = str_replace('col-md-', '', $menu['width_sub']);
						$int_width = (int)$int_width;
						if ($int_width <= 11)
						{
							$int_sm = $int_width + 1;
							$menu['width_sub_sm'] = 'col-sm-'.$int_sm;
						}
						else
							$menu['width_sub_sm'] = 'col-sm-12';
