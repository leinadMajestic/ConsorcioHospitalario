...
class WtMegamenuClass extends ObjectModel
{
	...
	public function getMenus()
	{
		...
		$kq = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE wt.active = 1 AND wl.id_shop = '.$id_shop.' AND wl.id_lang='.$id_lang.' ORDER BY wt.position ASC, wt.id_wtmegamenu ASC');
		return $kq;
	}
	...
	public function getPositionByMenu($id_menu)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			SELECT ms.`position`
			FROM '._DB_PREFIX_.'wtmegamenu_shop ms
			WHERE ms.`id_wtmegamenu` = '.$id_menu.' AND ms.`id_shop` = '.$id_shop);
	}
}