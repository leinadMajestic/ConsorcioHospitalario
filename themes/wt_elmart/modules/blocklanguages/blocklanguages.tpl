	...
	<div id="languages-block-top" class="languages-block">
		{foreach from=$languages key=k item=language name="languages"}
			{if $language.iso_code == $lang_iso}
				<div class="current">
					<span>
						<img src="{$base_dir}img/l/{$language.id_lang}.jpg" alt="{$language.name|escape:'html':'UTF-8'}"/></span>
				</div>
			{/if}
		{/foreach}
		<ul id="first-languages" class="languages-block_ul toogle_content">
			{foreach from=$languages key=k item=language name="languages"}
				...
				{if $language.iso_code != $lang_iso}
					...
					{/if}
				{/if}
						<img src="{$base_dir}img/l/{$language.id_lang}.jpg" alt="{$language.name|escape:'html':'UTF-8'}"/>
