										...
										<fieldset class="attribute_fieldset">
											...
											<div class="attribute_list">
												...
												{elseif ($group.group_type == 'color')}
														<ul id="{$group.group_name|lower|escape:'html':'UTF-8'}_to_pick_list" class="{if $group.group_name|lower != 'color'}wt-group-attribute {/if}wt-group-{$group.group_name|lower|escape:'html':'UTF-8'} clearfix">
														{assign var="default_colorpicker" value=""}
														{foreach from=$group.attributes key=id_attribute item=group_attribute}
															{assign var='img_color_exists' value=file_exists($col_img_dir|cat:$id_attribute|cat:'.jpg')}
															<li class="color-{$colors.$id_attribute.name|lower|escape:'html':'UTF-8'}{if $group.default == $id_attribute} selected{/if}">
																<a href="{$link->getProductLink($product)|escape:'html':'UTF-8'}" id="color_{$id_attribute|intval}" name="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" class="color_pick{if ($group.default == $id_attribute)} selected{/if}"{if !$img_color_exists && isset($colors.$id_attribute.value) && $colors.$id_attribute.value} style="background:{$colors.$id_attribute.value|escape:'html':'UTF-8'};"{/if} title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}">
																	{if $img_color_exists}
																		<img src="{$img_col_dir}{$id_attribute|intval}.jpg" alt="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" width="20" height="20" />
																	{else}
																		{if $group.group_name|lower != 'color'}
